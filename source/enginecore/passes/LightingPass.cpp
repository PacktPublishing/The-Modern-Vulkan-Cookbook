...
void LightingPass::init(VulkanCore::Context* context,
  ...
  samplerShadowMap_ = context_->createSampler(
      VK_FILTER_LINEAR, VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
      VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
      1.0f,  // enable these parameter if using sampler2DShadow
      true, VK_COMPARE_OP_LESS, "lighting pass shadow sampler");
  ...
  auto fragmentShader =
      context->createShaderModule((resourcesFolder / "lighting.frag").string(),
          ...
          .bindings_ =
              {
                  ...
                  VkDescriptorSetLayoutBinding{BINDING_BASECOLOR,
                                               VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                               1, VK_SHADER_STAGE_FRAGMENT_BIT},
                  VkDescriptorSetLayoutBinding{BINDING_DEPTH,
                                               VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                               1, VK_SHADER_STAGE_FRAGMENT_BIT},
                  VkDescriptorSetLayoutBinding{BINDING_POSITION,
                                               VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                               1, VK_SHADER_STAGE_FRAGMENT_BIT},
                  VkDescriptorSetLayoutBinding{BINDING_AMBIENTOCCLUSION,
                                               VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                               1, VK_SHADER_STAGE_FRAGMENT_BIT},
                  VkDescriptorSetLayoutBinding{BINDING_SHADOWDEPTH,
                                               VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                               1, VK_SHADER_STAGE_FRAGMENT_BIT},
              },
      },
  ...
  pipeline_->bindResource(GBUFFERDATA_SET, BINDING_DEPTH, 0, gBufferDepth_, sampler_);

  pipeline_->bindResource(GBUFFERDATA_SET, BINDING_POSITION, 0, gBufferPosition_,
                          sampler_);

  pipeline_->bindResource(GBUFFERDATA_SET, BINDING_AMBIENTOCCLUSION, 0, ambientOcclusion_,
                          sampler_);
  ...
  pipeline_->bindResource(GBUFFERDATA_SET, BINDING_SHADOWDEPTH, 0, shadowDepth_,
                          useSampler2DForShadows ? sampler_ : samplerShadowMap_);
...
void LightingPass::render(VkCommandBuffer commandBuffer, uint32_t index,
  ...
  context_->beginDebugUtilsLabel(commandBuffer, "Lighting Pass",
                                 {0.0f, 0.0f, 1.0f, 1.0f});
