#include "FullScreenColorNBlendPass.hpp"
...
void FullScreenColorNBlendPass::init(VulkanCore::Context* context,
  ...
  sampler_ = std::make_shared<VulkanCore::Sampler>(
      *context_, VK_FILTER_NEAREST, VK_FILTER_NEAREST,
      ...
      VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER, 1, "Full Screen Color & Blend Sampler");

  const auto resourcesFolder = std::filesystem::current_path() / "resources/shaders/";

  auto vertexShader = context->createShaderModule(
      (resourcesFolder / "fullscreenColorBlend.vert").string(),
      VK_SHADER_STAGE_VERTEX_BIT, "full screen color vertex");
  auto fragmentShader = context->createShaderModule(
      (resourcesFolder / "fullscreenColorBlend.frag").string(),
      VK_SHADER_STAGE_FRAGMENT_BIT, "full screen color fragment");
...
void FullScreenColorNBlendPass::render(VkCommandBuffer commandBuffer, uint32_t index,
  ...
  pipeline_->bindResource(0, 0, 0, src, sampler_);
