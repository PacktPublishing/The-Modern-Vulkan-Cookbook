...
class DepthPeeling final {
 public:
  explicit DepthPeeling(VulkanCore::Context* context) : context_{context} {}
  ...
  std::shared_ptr<VulkanCore::Texture> colorTexture() const {
    return colorTextures_[(numPeels_ - 1) % 2];
  }
 ...
 private:
  ...
  void initColorTextures(uint32_t numPeels, VkFormat colorTextureFormat);
 ...
 private:
  ...
  std::vector<std::shared_ptr<VulkanCore::Texture>> colorTextures_;

  // Depth textures (2x)
  std::array<std::shared_ptr<VulkanCore::Texture>, 2> depthTextures_;
