...
#pragma region Load model
  {
    const auto commandBuffer = commandMgr.getCmdBufferToBegin();
    {
      ...
      const std::shared_ptr<EngineCore::Model> duck =
...
#pragma region Shaders
  ...
  const auto vertexShader =
      context.createShaderModule((resourcesFolder / "gpuText.vert").string(),
                                 VK_SHADER_STAGE_VERTEX_BIT, "main vertex");
  const auto fragmentShader =
      context.createShaderModule((resourcesFolder / "gpuText.frag").string(),
                                 VK_SHADER_STAGE_FRAGMENT_BIT, "main fragment");

  const auto vertexShaderGPULines =
      context.createShaderModule((resourcesFolder / "gpuTextDraw.vert").string(),
                                 VK_SHADER_STAGE_VERTEX_BIT, "main vertex");
  const auto fragmentShaderGPULines =
      context.createShaderModule((resourcesFolder / "gpuTextDraw.frag").string(),
                                 VK_SHADER_STAGE_FRAGMENT_BIT, "main fragment");
...
#pragma endregion
    ...
    vkCmdFillBuffer(commandBuffer, gpuLineBuffer->vkBuffer(), sizeof(uint32_t),
                    sizeof(uint32_t), 0);
