...
void addLine(vec3 v0, vec3 v1, vec4 c0, vec4 c1) {
  const uint idx = atomicAdd(lineBuffer.cmd.instanceCount, 1);
...
void printDigit(int digit, uint linenum, uint column) {
  const float pixelWidth = 30;            // pixels
  const float pixelHeight = 50;           // pixels
  const float horSpaceNDC = 10.0 / 1600;  // pixels
  const float verSpaceNDC = 10.0 / 1200;  // pixels
  const float charWidthNDC = pixelWidth / 1600;
  const float charHeightNDC = pixelHeight / 1200;
  const float colx =
      column * (charWidthNDC + horSpaceNDC) + charWidthNDC + horSpaceNDC;
  const float coly =
      linenum * (2 * charHeightNDC + verSpaceNDC) + charHeightNDC + verSpaceNDC;
...
void parse(float val, uint decimals) {
  ...
  if (decimals > 0) {
    ...
    const int decimal = int(val * tens - intPart * tens);
