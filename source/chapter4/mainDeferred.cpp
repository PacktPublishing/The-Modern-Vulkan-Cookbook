...
#include "enginecore/passes/HierarchicalDepthBufferPass.hpp"
#include "enginecore/passes/LightingPass.hpp"
#include "enginecore/passes/NoisePass.hpp"
#include "enginecore/passes/SSAOPass.hpp"
#include "enginecore/passes/SSRIntersectPass.hpp"
#include "enginecore/passes/ShadowPass.hpp"
...
#pragma endregion
  ...
  NoisePass noisePass;
  noisePass.init(&context);

  HierarchicalDepthBufferPass hierarchicalDepthBufferPass;
  hierarchicalDepthBufferPass.init(&context, gbufferPass.depthTexture());

  SSAOPass ssaoPass;
  ssaoPass.init(&context, gbufferPass.depthTexture());

  FullScreenPass fullscreenPass;
  fullscreenPass.init(&context, {swapChainFormat});

  LightData lightData;
  lightData.setLightPos(glm::vec3(-3.5, 24.0, 2.0));
  ...
  lightPass.init(&context, gbufferPass.normalTexture(), gbufferPass.specularTexture(),
                 ...
                 gbufferPass.depthTexture(), ssaoPass.ssaoTexture(),
                 shadowPass.shadowDepthTexture());

  SSRIntersectPass ssrPass;
  ssrPass.init(&context, &camera, gbufferPass.normalTexture(),
               gbufferPass.specularTexture(), lightPass.lightTexture(),
  ...
  std::shared_ptr<VulkanCore::Pipeline> shadowPipeline = shadowPass.pipeline();

  auto textureReadyCB = [&gbufferPipeline, &shadowPipeline, &textures](int textureIndex,
                                                                       int modelId) {
    gbufferPipeline->bindResource(TEXTURES_SET, BINDING_0, 0,
                                  {textures.begin() + textureIndex, 1}, nullptr,
                                  textureIndex);
    shadowPipeline->bindResource(TEXTURES_SET, BINDING_0, 0,
                                 {textures.begin() + textureIndex, 1}, nullptr,
                                 textureIndex);
  };
...
#pragma region Pipeline initialization
  ...
  shadowPipeline->bindResource(CAMERA_SET, BINDING_0, 0, lightCamBuffer.buffer(0), 0,
                               sizeof(UniformTransforms),
                               VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
  shadowPipeline->bindResource(CAMERA_SET, BINDING_0, 1, lightCamBuffer.buffer(1), 0,
                               sizeof(UniformTransforms),
                               VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
  shadowPipeline->bindResource(CAMERA_SET, BINDING_0, 2, lightCamBuffer.buffer(2), 0,
                               sizeof(UniformTransforms),
                               VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
  shadowPipeline->bindResource(STORAGE_BUFFER_SET, BINDING_0, 0,
                               {buffers[0], buffers[1], buffers[3],
                                buffers[2]},  // vertex, index, indirect, material
                               VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
  shadowPipeline->bindResource(TEXTURES_SET, BINDING_0, 0,
                               {textures.begin(), textures.end()});
  shadowPipeline->bindResource(SAMPLER_SET, BINDING_0, 0, {samplers.begin(), 1});
...
#pragma endregion
  ...
  while (!glfwWindowShouldClose(window_)) {
    ...
    if (imguiMgr) {
      ...
      imguiMgr->setDisplayShadowMapTexture(displayShadowMap);
      bool valueChanged = displayShadowMap != imguiMgr->displayShadowMapTexture();
      displayShadowMap = imguiMgr->displayShadowMapTexture();
      if (valueChanged) {
        if (displayShadowMap) {
          auto shadowTexture = shadowPass.shadowDepthTexture();

          fullscreenPass.pipeline()->bindResource(0, 0, 0, {&shadowTexture, 1},
                                                  samplers[0]);
        } else {
