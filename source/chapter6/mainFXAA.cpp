...
#include "enginecore/passes/FXAAPass.hpp"
...
#pragma region Multisample Intermediary Texture
  ...
  for (uint32_t i = 0; i < context.swapchain()->numberImages(); ++i) {
    multisampleTextures[i] = context.createTexture(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, false, VK_SAMPLE_COUNT_1_BIT,
        "main render multisample image " + std::to_string(i));
  }
#pragma endregion
...
#pragma region Swapchain Framebuffers Initialization
  for (size_t index = 0; index < context.swapchain()->numberImages(); ++index) {
    swapchain_framebuffers[index] = context.createFramebuffer(
        renderPass->vkRenderPass(), {multisampleTextures[index], depthTexture}, nullptr,
        nullptr,
...
#pragma region FXAA Pass
    fxaaPass.render(commandBuffer, index, multisampleTextures[index],
                    context.swapchain()->texture(index));
#pragma endregion
