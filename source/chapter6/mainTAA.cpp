...
#include "enginecore/passes/TAAComputePass.hpp"
#include "imgui.h"
...
#pragma endregion
  ...
  TAAComputePass taaPass;
  taaPass.init(&context, gbufferPass.depthTexture(), gbufferPass.velocityTexture(),
               gbufferPass.baseColorTexture());

  auto textureToDisplay = taaPass.colorTexture();
...
#pragma endregion
  ...
  while (!glfwWindowShouldClose(window_)) {
    ...
    bool isCamMoving = !(transform.prevViewMat == transform.view);
    ...
    gbufferPass.render(commandBuffer, index,
                       ...
                       sizeof(EngineCore::IndirectDrawCommandAndMeshData), true);

    taaPass.doAA(commandBuffer, frameIndex, isCamMoving);
